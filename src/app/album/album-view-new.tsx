...
const GAMES = ["Kingdom TCG"] as const;
...
function CardModal({
  ...
  onList: (type: "sale" | "trade" | "both") => void;
}) {
      ...
      <div className="w-full max-w-sm bg-surface-900 border border-surface-700 rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="flex items-start gap-3 p-4 border-b border-surface-800">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-surface-400 mt-0.5">{card.category}{card.level != null ? ` · Nv. ${card.level}` : ""}</p>
        ...
        <div className="p-4 border-b border-surface-800">
          <p className="text-xs text-surface-400 mb-3 font-medium uppercase tracking-wide">Mi colección</p>
          ...
          {isOwned && quantity > 1 && (
            <p className="text-[10px] text-amber-400 mt-2 flex items-center gap-1">
              <Copy className="h-3 w-3" /> Tenés {quantity - 1} cop{quantity - 1 === 1 ? "ia" : "ias"} repetida{quantity - 1 === 1 ? "" : "s"}
            </p>
          )}
        </div>
        ...
        {isOwned && !listed && (
          <div className="p-4">
            <p className="text-xs text-surface-400 mb-3 font-medium uppercase tracking-wide">Publicar carta</p>
            {!listType ? (
              ...
              <div className="space-y-2">
                ...
                <Input
                  placeholder="Nota opcional (condición, etc.)"
                  value={listNote}
                ...
                <button
                  onClick={handleList}
                  disabled={listing || ((listType === "sale" || listType === "both") && !price)}
        ...
        {listed && (
          <div className="p-4 text-center">
            ...
            <p className="text-xs text-surface-400 mt-1">
              ...
              <Link href="/trades" className="text-primary-400 underline" onClick={onClose}>
                Intercambios
              </Link>
            </p>
          </div>
            ...
            <button
              ...
              className={cn(
                ...
                isOwned
                  ...
                  : "border-surface-700/50 bg-surface-900 opacity-50 hover:opacity-75 hover:border-surface-600",
                isLoading && "opacity-40 cursor-wait"
              )}
            >
              <div className="relative aspect-5/7 bg-surface-800 overflow-hidden">
                {single?.image_url ? (
                  <Image src={single.image_url} alt={card.name} fill className={cn("object-cover transition-all", !isOwned && "grayscale")} sizes="90px" />
                ) : (
...
const CoverPage = forwardRef<HTMLDivElement, { ownedCount: number; totalCards: number }>(
  function CoverPage({ ownedCount, totalCards }, ref) {
    ...
    return (
      <div
        ref={ref}
        className="bg-linear-to-br from-primary-950 via-surface-950 to-surface-950 border border-primary-800/40 flex flex-col items-center justify-center h-full select-none p-6 text-center"
      >
        {/* Kingdom TCG logo */}
        <div className="mb-4 w-32 h-16 relative">
          <Image
            src="https://kingdom-tcg.com/kingdom-tcg-tm-logo.svg"
            alt="Kingdom TCG"
            fill
        ...
        <h2 className="text-base font-bold text-surface-100 mb-1">Álbum Digital</h2>
        <div className="w-full bg-surface-800 rounded-full h-1.5 mb-2 mt-4">
          <div
...
const BackCover = forwardRef<HTMLDivElement>(function BackCover(_, ref) {
  return (
    <div
      ref={ref}
      className="bg-linear-to-br from-surface-950 to-primary-950 border border-primary-900/30 flex flex-col items-center justify-center h-full select-none"
    >
      <div className="w-24 h-12 relative opacity-20">
        <Image
          src="https://kingdom-tcg.com/kingdom-tcg-tm-logo.svg"
          alt="Kingdom TCG"
          fill
...
export function AlbumView({ albumMap: initialAlbumMap, singlesMap }: AlbumViewProps) {
  ...
  const [selectedGame] = useState<string>("Kingdom TCG");
  ...
  return (
    <PageLayout title="Álbum Digital" description="Las cartas del catálogo que tenés en tu colección">
      ...
      {/* Game filter (extensible) */}
      <div className="flex items-center gap-2 mb-5">
        <span className="text-xs text-surface-500">Juego:</span>
        {GAMES.map((g) => (
          <span
            key={g}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-600 text-white text-xs font-medium"
          >
            <div className="h-3 w-3 relative">
              <Image src="https://kingdom-tcg.com/kingdom-tcg-tm-logo.svg" alt={g} fill className="object-contain" unoptimized />
            </div>
            {g}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between mb-5">
        <p className="text-xs text-surface-500 hidden sm:block">
          {viewMode === "flip"
            ? "Hacé clic en una carta para ver detalles • Usá las flechas para pasar páginas"
            : "Hacé clic en una carta para ver detalles y publicarla"}
        </p>
      ...
      {viewMode === "grid" && (
        <>
          ...
          <div className="flex items-center gap-3 mb-5 flex-wrap text-xs text-surface-500">
            ...
            <span className="flex items-center gap-1.5">
              <div className="h-3 w-3 rounded border-2 border-surface-700 opacity-50" /> No la tenés (en gris)
            </span>
            <span className="text-surface-600">· Hacé clic para ver detalles</span>
          </div>
      ...
      {modal && (
        <CardModal
          ...
          onSetQty={(qty) => {
            handleSetQty(modal.card.code, qty);
            setModal((m) => m ? { ...m } : null);
          }}
          onList={(type) => {
